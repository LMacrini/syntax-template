...
import { Image, Link, MediaIcon, useLoadProfileBody, stripTags, getPageProfile } from '@uniwebcms/module-sdk';
            ...
            <li key={index}>
                ...
                <ul role='list' className='mt-3 flex gap-x-3'>
                    {links.map((link, index) => {
                        ...
                        return (
                            <Link key={index} to={link.href} target='_blank' title={website.localize(linkTitle)}>
            ...
            <li key={index}>
                ...
                <ul role='list' className='mt-3 flex gap-x-3'>
                    {mediaLinks.map((link, index) => {
                        ...
                        const type = link.type || website.localize({ en: 'website', fr: 'site web' });
                        ...
                        return (
                            <Link key={index} to={url} target='_blank' title={website.localize(linkTitle)}>
                                <MediaIcon type={type} size='5' />
                            </Link>
                        );
                    })}
        ...
        <Container>
            <div className='mx-auto max-w-7xl px-6 lg:px-8'>
                <div>
                    ...
                    {subtitle ? (
                        <p className='mt-2 text-lg leading-8 sm:text-xl text-text-color-80'>{stripTags(subtitle)}</p>
                    ) : null}
                </div>
                <ul
                    role='list'
                    ...
                    {members.length ? (
                        <ProfilesRenderer profiles={members} aspectRatio={aspect_ratio} website={website} />
                    ) : items.length ? (
