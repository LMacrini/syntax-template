            ...
            <div className='relative max-w-7xl mx-auto'>
                <div
                    ...
                    <div className='relative lg:max-w-3xl w-full flex flex-col px-6 md:px-8 py-12 md:py-16 lg:py-20'>
                        {pretitle ? (
                            <h3 className='mb-2 font-medium text-xl md:text-2xl lg:text-3xl'>{stripTags(pretitle)}</h3>
                        ) : null}
                        ...
                        <SafeHtml className='mt-8 text-base md:text-lg space-y-2' value={paragraphs} />
                        {link ? (
                            <div className='mt-5 flex items-center justify-center'>
                                <Link to={website.makeHref(link.href)} className='text-base font-semibold leading-6'>
                                    {link.label} <span className='!text-[inherit]'>→</span>
                                </Link>
                            </div>
                        ) : null}
                        {show_back_drop && (
                            <div className='absolute inset-0 bg-bg-color opacity-75' style={{ zIndex: -1 }}></div>
                        )}
                    </div>
                </div>
            </div>
