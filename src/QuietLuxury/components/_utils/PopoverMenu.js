...
export default function PopoverMenu(props) {
    ...
    return (
        <Popover className={'relative'}>
            {({ open }) => (
                <div onMouseEnter={() => onHover('onMouseEnter')} onMouseLeave={() => onHover('onMouseLeave')}>
                    <Popover.Button className={`${triggerClassName}`} ref={buttonRef} onClick={() => handleClick(open)}>
                        {trigger}
                    </Popover.Button>
                    <Transition
                        as={Fragment}
