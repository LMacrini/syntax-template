...
export default function Searcher(props) {
    ...
    return (
        <div
            className={twMerge(
                'relative w-48 px-2 2xl:px-2.5 py-1.5 flex items-center space-x-2 2xl:space-x-2 border border-text-color-20 bg-text-color-10 rounded-lg overflow-hidden hover:bg-text-color-0 focus-within:bg-text-color-0 shadow'
            )}
            onClick={() => inputRef.current.focus()}>
            <BsSearch className='flex-shrink-0 w-[18px] h-[18px] text-text-color-60' aria-hidden='true' />
            <input
                ref={inputRef}
                type='text'
                name='search'
