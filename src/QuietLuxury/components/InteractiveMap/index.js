...
import { getAddressesFromCards, getLocationsFromInput, mergeLocationAndAddress } from '../_utils/location';
    ...
    return (
        <Container className='!py-0 w-screen h-screen'>
            <div className={'w-full h-full'}>
                <div className='absolute top-5 z-[2] group' style={{ width: widthStyle.input, left: leftStyle.input }}>
                    <Searcher
                        website={website}
                ...
                <Map
                    ...
                    zoom={zoom}
                    initialCenter={sortedLocations[0]?.location?.geo}></Map>
            </div>
        </Container>
    );
}
                    ...
                    <li key={`${profileTitle}_${title}_${index}`}>
                        <div
                            ...
                            <div className='flex-grow'>
                                ...
                                <p className='text-base font-normal text-neutral-700'>{profileTitle}</p>
                                <p className='mt-2 text-sm font-medium !text-neutral-900'>{address}</p>
                                {dateRange ? <p className='mt-1 text-sm text-neutral-700'>{dateRange}</p> : null}
                            </div>
                        </div>
                    </li>
                );
