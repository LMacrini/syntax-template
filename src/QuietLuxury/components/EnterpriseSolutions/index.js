...
export default function EnterpriseSolutions({ block, website }) {
    const { main } = block;
        ...
        <Container>
            {banner ? (
                <div className='absolute inset-0'>
                    ...
                    <div
                        className='absolute inset-0 !bg-neutral-200 mix-blend-multiply opacity-60'
                        aria-hidden='true'
                    />
                </div>
            ) : null}
            <div className='mx-auto max-w-7xl px-6 md:px-8'>
                <div className='relative flex flex-col'>
                    ...
                    {subtitle ? (
                        <p className='mt-4 text-xl !text-neutral-800 md:text-2xl'>{stripTags(subtitle)}</p>
                    ) : null}
                </div>
                <div className='mt-12 grid grid-cols-1 gap-x-6 gap-y-8 lg:grid-cols-2 lg:gap-x-8 lg:gap-y-10'>
                    {items.map((item, index) => {
                        ...
                        return (
                            <div key={index} className='z-10 flex flex-col shadow-xl bg-bg-color-60 rounded-2xl'>
                                ...
                                {link ? (
                                    <div className='px-6 pt-3 pb-4 rounded-bl-2xl rounded-br-2xl md:px-8'>
                                        <Link to={website.makeHref(link.href)} className='text-base font-medium'>
                                            <span className='text-link-color'>{stripTags(link.label)}</span>
                                        </Link>
                                    </div>
                                ) : null}
                            </div>
