...
export default function PowerFeatures({ block }) {
    ...
    return (
        <Container className='!py-0'>
            <div className={`px-6 lg:px-8 max-w-5xl mx-auto pt-16 lg:pt-24 pb-4 text-center leading-[2.3rem]`}>
                ...
                {items.map((power, index) => {
                    ...
                    return (
                        <div key={index} className='py-12 sm:py-16 sm:mx-8'>
                            ...
                            <h3 className='my-4 text-2xl font-bold text-text-color'>{stripTags(title)}</h3>
                            <SafeHtml value={paragraphs} className='py-2 text-text-color-90' />
                            {link ? (
                                <Link to={link.href} className='mt-4 text-sm font-medium hover:underline'>
                                    {link.label}
                                </Link>
                            ) : null}
                        </div>
