...
import { AiFillCaretRight, AiFillCaretLeft, AiOutlineFullscreen, AiOutlineFullscreenExit } from 'react-icons/ai';
import { MdOutlineFormatListBulleted } from 'react-icons/md';
import { BiSearch, BiX } from 'react-icons/bi';
import { getLocationsFromInput } from '../_utils/location';
            ...
            <div className='w-full'>
                <div className='w-full h-10 border rounded-lg border-neutral-300 !shadow !shadow-neutral-300 bg-neutral-50 focus-within:bg-neutral-50 flex items-center justify-between space-x-1 py-1 px-2 group'>
                    <input
                        ...
                        placeholder={targetProfileTitle || website.localize({ en: 'Search', fr: 'Recherche' })}
                        onChange={(e) => {
                        ...
                        onFocus={() => {
                            if (!showDetail) {
                                ...
                                if ((showSidePanel && inProfileView) || (inProfileView && !showSidePanel)) {
                                    setShowMenu(true);
                                }
                                if (!inProfileView) {
                                    setShowSidePanel(true);
...
const Sidebar = (props) => {
    ...
    const content = inProfileView ? (
        ...
        <ProfileList {...{ locations: filteredLocations, website, map, showDetail, pathname, setSearchText }} />
    );
    ...
    return (
        <div className='relative w-full h-full flex'>
                    ...
                    return (
                        <div
                            ...
                            className={twJoin(
                                ...
                                active ? 'text-secondary-700' : 'text-neutral-500 hover:text-neutral-800'
                            )}
                            onClick={() => {
                                setFilter({
                                    ...filter,
            ...
            {showSidePanel && showMenu ? <div className='absolute inset-0 bg-neutral-800/70 z-0'></div> : null}
        </div>
    );
};
...
const ProfileViewer = memo(
    ({ type, id, tab }) => {
        ...
        useEffect(() => {
            if (!Object.keys(renderers).includes(type)) {
                const renderer = lazy(() => {
                    return uniweb.getProfileRenderer(`${type}/profile`).catch(() => ({ default: () => null }));
                });
    ...
    (prevProps, nextProps) => {
        return prevProps.id === nextProps.id && prevProps.tab === nextProps.tab && prevProps.type === nextProps.type;
    }
);
                    ...
                    <li key={`${profileTitle}_${title}_${index}`}>
                        <div
                            ...
                            onClick={() => {
                                ...
                                if (showDetail) {
                                    setSearchText(profileTitle);
                                    navigate(`${pathname}?type=${profile.contentType}&id=${profile.contentId}`);
                                }
                            }}>
                            ...
                            <div className='flex-grow'>
                                ...
                                <p className='mt-1 text-base font-normal text-secondary-600'>{title}</p>
...
export default function DynamicMap(props) {
    ...
    return (
        <Container className={twJoin(bigSize ? '!py-0 w-screen h-screen' : 'mx-auto max-w-7xl px-6 md:px-8')}>
            {!bigSize && title ? (
            ...
            <div
                ...
                <div className='absolute top-5 z-[2]' style={{ width: widthStyle.input, left: leftStyle.input }}>
                    <Searcher
                        website={website}
