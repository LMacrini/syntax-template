    ...
    return (
        <div className='w-full'>
            <div className='w-full flex flex-wrap items-center gap-4'>
                ...
                <p className='text-base sm:text-lg font-semibold text-heading-color'>{stripTags(title)}</p>
            </div>
            <p className='mt-2 text-md sm:text-base font-medium text-text-color-80'>{stripTags(subtitle)}</p>
            <div className='flex flex-wrap mt-6'>
                {mediaLinks.map((link, index) => {
                    ...
                    return (
                        <a key={index} className='mr-4' href={href} target='_blank' title={website.localize(linkTitle)}>
            ...
            {linkGroups.map((group, index) => {
                ...
                return (
                    <div
                        ...
                        <p className='text-base font-semibold uppercase text-text-color-70'>{stripTags(label)}</p>
                            ...
                            {links.map((link, index) => {
                                ...
                                return (
                                    <li key={index}>
                                        <Link
                                            to={href}
                                            className='text-base font-medium text-text-color hover:underline'>
                                            {stripTags(label)}
                                        </Link>
                                    </li>
                                );
                ...
                <div className='w-full max-w-xs'>
                    ...
                    <div className='border-b border-text-color-90 py-2 px-1'>
                        <input
                            ...
                            autoComplete='email'
                            required
                            className='bg-inherit block w-full text-base text-text-color placeholder:text-text-color-70 focus:outline-none'
            ...
            <div className='px-6 md:px-8'>
                <div className='relative mx-auto max-w-10xl grid gap-12 lg:grid-cols-6 lg:gap-18'>
                    <div className='col-span-2'>
                        <Branding title={title} subtitle={subtitle} logo={logo} mediaLinks={mediaLinks} />
                    </div>
                    <div
                        className={twJoin(
                    ...
                    {with_newsletter ? (
                        <div className={twJoin('relative col-span-2', with_back_to_top && 'pr-[100px]')}>
                            <Newsletter />
                        </div>
                    ) : null}
                    {with_back_to_top ? (
                        <>
                            <div
                                className={twJoin(
                                    ...
                                    with_newsletter || groupedLinks.length > 2 ? 'right-0' : 'right-0 lg:right-[15%]',
                                    with_newsletter ? 'row-start-3' : 'row-start-2'
                                )}>
                                <BackToTop size={with_newsletter || groupedLinks.length > 3 ? 'sm' : 'md'} />
                            </div>
                        </>
                    ) : null}
                </div>
