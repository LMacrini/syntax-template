...
import { getNextBlockContext } from '../_utils';
import './style.css';
...
const isActiveRoute = (activeRoute, current) => {
    return activeRoute === current || activeRoute === `${current}/index` || activeRoute === `${current}/[id]`;
};
    ...
    return (
        <div className={twMerge(wrapperClass, !initialPosition && sticky && '!shadow-2xl')}>
            <div
                ...
                {logo ? (
                    <Link to='' title="A link back to the site's homepage" className='w-fit max-w-[8rem]'>
                        <div className='h-[32px] lg:h-[44px] w-auto max-w-full'>{logo}</div>
                    </Link>
                ) : null}
                <nav
                    className={twJoin(
                        'hidden lg:flex 2xl:space-x-14 xl:space-x-10 lg:space-x-8 items-center w-full sm:h-10',
                        left_aligned ? 'justify-start ml-16' : 'justify-center'
                    )}>
                    {navigation.map((page, index) => {
                        if (page.child_items?.length) {
                            return <NavbarMenu key={index} {...page} activeRoute={activeRoute} theme={adaptiveTheme} />;
                        } else {
                            ...
                            return (
                                <Link
                                    ...
                                    className={twJoin(
                                        ...
                                        active
                                            ? 'text-link-color'
                                            : 'text-text-color-90 hover:text-text-color hover:scale-125 transition-transform duration-300'
                                    )}>
                                    {label}
                                </Link>
                            );
                ...
                <div className='flex items-center space-x-6 w-44 lg:w-32 justify-end'>
                    ...
                    <button
                        ...
                        <HiBars3 className='w-6 h-6 text-text-color-70 hover:text-text-color-90' aria-hidden='true' />
                    </button>
                </div>
                <Dialog as='div' open={mobileMenuOpen} onClose={setMobileMenuOpen} className={adaptiveTheme}>
                    <Dialog.Panel className='fixed inset-0 z-50 px-6 py-3 overflow-y-auto lg:hidden bg-bg-color'>
                        ...
                        <div className='mt-6 space-y-5'>
                            {navigation.map((page, index) => {
                                if (page.child_items?.length) {
                                    return <MobileNavbarMenu key={index} {...page} activeRoute={activeRoute} />;
                                } else {
                                    const { route, label } = page;
                                    const active = isActiveRoute(activeRoute, route);
    ...
    return (
        <div>
            {hasData ? (
                ...
                <p className={twMerge('px-3 py-2 text-xl font-semibold leading-7 text-text-color-70')}>{label}</p>
            )}
                    ...
                    return (
                        <div key={index} className={twJoin('')}>
                            {hasData ? (
                                ...
                                <p className='block text-lg font-medium leading-7 text-text-color-70'>{label}</p>
                            )}
                            {child_items?.length ? (
